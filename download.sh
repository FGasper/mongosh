...
if [ -n "$version" ]; then
    versions_selector='(.versions[] | select(.version == "'"$version"'"))'
else
    versions_selector='.versions[0]'
fi

jq_query=$(cat <<EOF
${versions_selector}.downloads[] |
select(
    .distro == "$os" and
    .arch == "$arch"
    $openssl_query
...
if [ -z "$url" ]; then
    if [ -n "$version" ]; then
        say "❓ No download found for $os on $arch (version $version)."
    else
